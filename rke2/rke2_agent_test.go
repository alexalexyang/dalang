...
func TestInstallRke2Agent(t *testing.T) {
	...
	deployFunc := func(ctx *pulumi.Context) error {
		...
		serverServer, err := hetzner.DeployServer(ctx, serverSSHKey, 1)
		if err != nil {
			t.Log("Error with DeployNetworkFunc: ", err)
			return err
		}
		...
		installServerRes, err := InstallServer(ctx, &serverConnectInfo, []pulumi.Resource{serverServer})
		if err != nil {
		...
		serverServer.Ipv4Address.ApplyT(func(ip string) string {
			serverChan <- ip
			return ip
		})
		...
		agentServer, err := hetzner.DeployServer(ctx, serverSSHKey, 1)
		if err != nil {
			t.Log("Error with DeployNetworkFunc: ", err)
			return err
		}

		t.Log("Agent server deployed: ", agentServer)

		agentConnectInfo := remote.ConnectionArgs{
			Host:       agentServer.Ipv4Address,
			Port:       pulumi.Float64(22),
			User:       pulumi.String("root"),
			PrivateKey: serverKeyPair.Private,
		}
		...
		agentServer.Ipv4Address.ApplyT(func(ip string) string {
		...
		_, agentStatus, err := StartAgent(ctx, &agentConnectInfo, []pulumi.Resource{agentServer}, serverIp, *serverToken)
		if err != nil {
